...
public class MongoSink extends AbstractSink implements Configurable {
	...
	public static final String GREP = "search";
 ...
 */
	...
	public void configure(Context context) {
		...
		logger.info("Start read grep message  {}.....................",
				context.getString(GREP, ""));
		grep = context.getString(GREP, "");

		authentication_enabled = context.getBoolean(AUTHENTICATION_ENABLED,
		...
		model = CollectionModel.valueOf(context.getString(MODEL,
				CollectionModel.single.name()));
		...
		logger.info(
				"MongoSink {} context { host:{}, port:{}, authentication_enabled:{}, username:{}, password:{}, model:{}, dbName:{}, collectionName:{}, batch: {}, autoWrap: {}, wrapField: {}, timestampField: {}     grep:{}}    ",
				new Object[] { getName(), host, port, authentication_enabled,
						username, password, model, dbName, collectionName,
						batchSize, autoWrap, wrapField, timestampField, grep });
	}
	...
	private void saveEvents(Map<String, List<DBObject>> eventMap) {
		...
		for (String eventCollection : eventMap.keySet()) {
			...
			if (authentication_enabled) {
				boolean authResult = db.authenticate(username,
			...
			// db.getCollection(collectionName).insert(docs,
			// WriteConcern.NORMAL).getLastError();
			...
			for (DBObject a : docs) {
				...
				logger.info("==========" + "".equals(grep)
						+ "========================="
						+ (tempa.indexOf(grep) > 0));

				logger.info("==========" + grep);

				DBObject _b = Nginx.getJson(tempa, grep);

				if (_b != null) {
					docs_heiji.add(_b);
				}
			}
			if(docs_heiji!=null&&docs_heiji.size()>0){
				//没有消息就不插入。
				CommandResult result = db.getCollection(collectionName)
						.insert(docs_heiji, WriteConcern.NORMAL).getLastError();
				if (result.ok()) {
		...
		for (String eventCollection : eventMap.keySet()) {
			...
			for (DBObject doc : docs) {
				...
				CommandResult result = collection.update(query, doc, true,
						false, WriteConcern.NORMAL).getLastError();
				if (result.ok()) {
					String errorMessage = result.getErrorMessage();
					if (errorMessage != null) {
	...
	private void processEvent(Map<String, List<DBObject>> eventMap, Event event) {
		...
		default:
			logger.error(
					"can't support model: {}, please check configuration.",
					model);
		}
	}

	private void putDynamicEvent(Map<String, List<DBObject>> eventMap,
			Event event) {
	...
	private void putSingleEvent(Map<String, List<DBObject>> eventMap,
			Event event) {
		String eventCollection;
